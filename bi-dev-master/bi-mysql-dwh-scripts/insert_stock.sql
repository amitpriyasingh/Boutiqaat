...
create temporary table complete_sku as select distinct i.PRODN sku, i.PAR_CODE barcode, i.PRODNAME_E sku_name, i.last_grn_date last_grn_date, cat1.NAME_E cat1, cat2.NAME_E cat2, cat3.NAME_E cat3, cat4.NAME_E cat4, br.NAME_E brand, ptm.purchase_type, i.VENDOR supplier_id, 1 store_id, ip.qty quantity, ip.location location,
    r2.grn_no grn_no,
    ...
    LEFT JOIN brand_purchase_type_master ptm ON ptm.brand_name=br.NAME_E
...
create temporary table sku_gross as select distinct sku, 
    ...
    FROM aoi where sku is not null group by sku ;
