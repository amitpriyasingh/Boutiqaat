...
WHERE order_inserted_on_utc > @max_order_inserted_on_utc OR (order_inserted_on_utc = @max_order_inserted_on_utc AND isl.ItemId > @max_item_id );

update aoi.order_details items join
(select items.item_id, items.celebrity_id, celeb_master.account_manager, celeb_master.celebrity_code, celeb_master.celebrity_name from (select * from aoi.order_details WHERE order_inserted_on_utc > @max_order_inserted_on_utc OR (order_inserted_on_utc = @max_order_inserted_on_utc AND item_id > @max_item_id ))items left join
...
DELETE from aoi.order_details where item_id=0;
COMMIT;
