...
MIN(IF(StatusId IN (20,36,22,12,14,23,25,13), InsertedOn,NULL)) returned_at,
...
update aoi.order_details d JOIN (select ItemId, InsertedOn from (select ItemId, COALESCE(UpdatedOn,InsertedOn) InsertedOn, Deleted from (select * from OFS.HoldOrders order by Id desc) ordered_ho GROUP BY ItemId) ho WHERE Deleted=0)held_items ON held_items.ItemId=d.item_id
SET d.order_status_id = 10, d.order_status = 'Hold', d.status_at = CONVERT_TZ(held_items.InsertedOn,'+00:00','+03:00');

update aoi.order_details d JOIN OFS.CancelledOrders cancelled_items ON cancelled_items.ItemId=d.item_id
