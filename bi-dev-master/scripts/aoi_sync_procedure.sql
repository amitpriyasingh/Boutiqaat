  DELIMITER //
  CREATE PROCEDURE dow_aoi()
  BEGIN
          set @center := 1;
          WHILE @center < 11 DO
                  select max(order_number) into @last_order from aoi where branch_id = @center;
                  insert into
                aoi_sample ( order_date, yearmonth , order_year , order_week , order_month , branch_id , store_id , order_number , device_type , payment_method_id , payment_method , line_id , sku , sku_name , sku_barcode , item_cost , new_price , sales_price , unit_price , last_order_price , category1 , category2, category3 , category4 , brand_id , brand_name , box_id , box_price , box_discount , box_name , discount , quantity , box_quantity , item_total , shipping_charge , order_discount , order_total , order_gmv , customer_name , customer_telephone , order_mobile , is_cancelled , is_returned , is_b_fulfilled , order_ref_no , celebrity_name , celebrity_id , celebrity_price , celebrity_commission , order_region , city , country , is_celebrity , currency_id , currency , currency_rate , ex_rate , awbstatus , awbnumber , return_awbnumber , driver_id , runsheet_no , non_inventory , status , status_group,supplier_id , supplier_name , supplier_acc_no , soh , sales_period , last_grn_date, ageing, ageing_month
                )
                (SELECT 
                ISSUE_1.VDATE,
                ...
                WHERE ISSUE_1.BILNO > @last_order
                  AND ISSUE_1.PCENTER = @center
                  AND ISSUE_1.BILNO = ISSUE_2.BILNOAND ISSUE_1.CODE = ISSUE_2.CODE
                ...
                WHERE ISSUE_1.BILNO > @last_order
                  AND ISSUE_1.PCENTER = @center
                  AND ISSUE_1.BILNO = ISSUE_2.BILNO
