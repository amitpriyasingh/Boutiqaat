...
create table cohort_kwd select customer_id, 
...
from (select sales_order_grid.customer_id, sales_order_grid.increment_id order_id, sales_order_grid.grand_total*curr.ex_rate gmv, DATE(sales_order_grid.created_at) order_date, DATE(first_order_date) first_order_date from sales_order_grid  join customer_first_order on sales_order_grid.customer_id = customer_first_order.customer_id left join CURRANCY curr on curr.CURRENCY_UNIT_E = sales_order_grid.order_currency_code) e group by customer_id;
