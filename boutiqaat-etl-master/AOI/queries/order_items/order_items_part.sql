...
CREATE TEMP TABLE tmp_order_items(
    ...
    order_item_index	VARCHAR(50),
    item_id INTEGER,
    batch_id BIGINT,
    awbno	VARCHAR(20),
    sku	VARCHAR(30),
    sku_name	VARCHAR(2048),
...
copy tmp_order_items from '{{S3PATH}}'
...
DELETE FROM aoi.order_items WHERE order_at >= '{{PDATE}}';
INSERT INTO aoi.order_items
SELECT order_number,app_order_number,order_item_index,item_id,batch_id,awbno,sku,sku_name,category1,category2,brand,celebrity_code,celebrity_id,celebrity_name,account_manager,quantity,order_currency,exchange_rate,net_sale_price,rrp,list_price,shipping_charge,cod_charge,allocated_order_count,order_date,order_month,first_order_date,first_order_month,order_at,order_date_utc,order_at_utc,order_type,order_category,payment_method,payment_gateway,customer_id,phone_no,is_first_order,	customer_ltoc,	billing_phone_no,billing_country,shipping_phone_no,shipping_country,net_sale_price_kwd,shipping_charge_kwd,cod_charge_kwd,order_status_id,order_status,status_grouped_mgmt,status_grouped_ops,last_activity,status_at,cancelled_at,confirmed_at,readytoship_at,picked_at,order_allocated_at,packed_at,shipped_at,delivered_at,returned_at,batch_inserted_at,order_inserted_on_utc
