...
select 
    ...
    '1' as page_id, 
    'Boutiqaat Mainsite' as page_name, 
...
(
    ...
    WHERE COALESCE((SELECT value.string_value FROM UNNEST(event_params)WHERE key = 'PageType'),
            (SELECT value.string_value FROM UNNEST(event_params)WHERE key = 'page_type')) in ('Listing Page','Search_result Page') 
    AND COALESCE((SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'PageName'),
            (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'page_name')) not like '%Celebrity%'
) as non_celebrity_pages
