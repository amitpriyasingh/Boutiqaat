BEGIN;
DROP TABLE IF EXISTS sandbox.customer_retention;
with fist_last_order_country as
(
select phone_no, order_currency, order_at,order_date,
...
from aoi.order_items where lower(order_category) != 'celebrity'
),
most_orders_country as 
(
select phone_no,order_currency, count(order_number) as total_countrywise_orders,
DENSE_RANK () over (partition by phone_no order by count(order_number) desc) as max_order_country_rank
from aoi.order_items where order_status not like '%Cancel%' and order_status not like '%Ret%' and lower(order_category) != 'celebrity'
group by 1,2 
),
most_shopped_category1 as 
(
select phone_no, category1,count(order_number) as total_category1_orders,
DENSE_RANK () over (partition by phone_no order by count(order_number) desc) as max_order_cat1_rank 
from aoi.order_items where lower(order_category) != 'celebrity' group by 1,2
),
most_shopped_category2 as 
(
select phone_no, category2,count(order_number) as total_category2_orders,
...
from aoi.order_items where lower(order_category) != 'celebrity' group by 1),
transactions as 
(
select distinct trans.phone_no, COALESCE (trans.order_at,trans.order_date) as order_at,
...
from (select distinct phone_no, order_at,order_date from aoi.order_items where lower(order_category) != 'celebrity'
) trans --where trans.phone_no is not null and trans.order_at is not null
order by trans.phone_no
)-----------------
select i.phone_no,foc.acquisition_date,foc.last_transaction_date,
...
orders.orders_10days,orders.orders_20days,orders.orders_30days,orders.orders_60days, orders.orders_90days,orders.orders_greater_than_30days,
orders.orders_10days_100days,orders.orders_20days_110days,orders.orders_30days_120days,
orders.orders_180days, orders.orders_10days_190days,orders.orders_20days_200days,orders.orders_30days_210days,orders.orders_365days, 
orders.orders_100days_190days, orders.orders_110days_200days, orders.orders_120days_210days,
orders.orders_this_month,orders.orders_last_month,orders.orders_second_last_month,orders.orders_third_last_month,
...
orders.grs_rev_10days,orders.grs_rev_20days,orders.grs_rev_30days,
orders.grs_rev_60days,orders.grs_rev_90days,orders.grs_rev_180days,orders.grs_rev_365days,orders.gross_revenue_life_time,
orders.grs_rev_10days_exclude_can_ret,orders.grs_rev_20days_exclude_can_ret,orders.grs_rev_30days_exclude_can_ret,
orders.grs_rev_60days_exclude_can_ret,orders.grs_rev_90days_exclude_can_ret,
orders.grs_rev_180days_exclude_can_ret,orders.grs_rev_365days_exclude_can_ret,orders.grs_rev_lifetime_exclude_can_ret,
...
orders.total_orders_till_date, rev_rank.cust_rev_rank,
...
into sandbox.customer_retention --drop view analytics.customer_ret -- drop table sandbox.customer_retention
from 
(select distinct phone_no from aoi.order_items) i
left join 
(select distinct phone_no, first_order_at::date as acquisition_date, last_order_at::date as last_transaction_date,
listagg(distinct order_currency,',') as first_order_country
from fist_last_order_country where first_order_at = COALESCE(order_at,order_date) group by 1,2,3) foc on foc.phone_no = i.phone_no
left join 
(select distinct phone_no, listagg(distinct order_currency,',') as last_order_country from fist_last_order_country 
where last_order_at = COALESCE(order_at,order_date) group by 1) as loc on loc.phone_no = i.phone_no
left join 
(
select distinct phone_no, listagg(order_currency,',') as most_order_country from most_orders_country where max_order_country_rank = 1 group by 1
) as moc on moc.phone_no = i.phone_no
left join 
(
select phone_no,listagg(distinct category1,',') as most_shopped_cat1
from most_shopped_category1 where max_order_cat1_rank = 1 
group by 1 
) as most_cat1 on most_cat1.phone_no = i.phone_no
left join 
(
select phone_no,listagg(distinct category1,',') as second_most_shopped_cat1
from most_shopped_category1 where max_order_cat1_rank = 2 
group by 1 
) as second_most_cat1 on second_most_cat1.phone_no = i.phone_no
--
left join 
(
select phone_no,listagg(category2,',') as most_shopped_cat2
from most_shopped_category2 where max_order_cat2_rank = 1
group by 1
) as most_cat2 on most_cat2.phone_no = i.phone_no
left join 
(
select phone_no,listagg(category2,',') as second_most_shopped_cat2
from most_shopped_category2 where max_order_cat2_rank = 2 
group by 1
) as second_most_cat2 on second_most_cat2.phone_no = i.phone_no
left join orders on orders.phone_no = i.phone_no
left join (select phone_no, gross_revenue_life_time, 
RANK() over (order by gross_revenue_life_time desc) as cust_rev_rank
from orders) as rev_rank on rev_rank.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = 1) as gap_first_trans on gap_first_trans.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = 2) as gap_second_trans on gap_second_trans.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = 3) as gap_third_trans on gap_third_trans.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = 4) as gap_fourth_trans on gap_fourth_trans.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = total_count) as gap_last_trans on gap_last_trans.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = total_count-1) as gap_second_last_trans on gap_second_last_trans.phone_no = i.phone_no
left join (select distinct phone_no,order_at,days_gap from transactions where trans_rank = total_count-2) as gap_2nd_last_3rd_last_trans on gap_2nd_last_3rd_last_trans.phone_no = i.phone_no;
COMMIT;